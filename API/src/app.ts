...
function sanitizeCharacterInput(req: Request, res: Response, next: NextFunction)
{
  req.body.sanitizeCharacterInput = {
    ...
    characterClass: req.body.characterClass.trim(),
...
app.put('/api/characters/:id', sanitizeCharacterInput, (req, res) => { // Update characters - no le encuentro mucho sentido, pero anda - el profe lo hizo buscando el index
  const character = characters.find(c => c.id === (req.params.id));
  if (!character) {
    res.status(404).send({ error: 'Character not found' });
    return;
